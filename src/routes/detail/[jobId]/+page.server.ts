...
export const load: PageServerLoad = async ({ params }) => {
	...
	try {
		...
		const result = await page.evaluate(() => {
			...
			const about = job.querySelectorAll('.JobDescription > .description > div')[0]?.innerHTML?.trim();
			const benefit = job.querySelectorAll('.JobDescription > .description > div')[1]?.innerHTML?.trim();
			const detail = job.querySelectorAll('.JobDescription > .description > div')[2]?.innerHTML?.trim();
